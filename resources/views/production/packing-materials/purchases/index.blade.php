...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
          ...
          if (result.value) {
            window.location.href="{{url('delete-pmpurchase/')}}/"+id;
            Swal.fire(
                ...
                <div class="card-body">
                   <div class="table-responsive">
                       <table id="del-multiple" class="table table-responsive table-striped display nowrap" style="width: 100%; font-size: 14px;">
                            ...
                            <tbody>
                                @foreach($pmpurchases as $index => $pmpurchase)
                                <tr>
                                    <td>{{$pmpurchase->id}}</td>
                                    <td>{{date('d-m-Y', strtotime($pmpurchase->date))}}</td>
                                    @if(!is_null($pmpurchase->supplier_id))
                                    <td><a href="{{url('pm-purchase-items/'.encrypt($pmpurchase->id))}}">{{App\Models\Supplier::find($pmpurchase->supplier_id)->name}}</a></td>
                                    @else
                                    <td><a href="{{url('pm-purchase-items/'.encrypt($pmpurchase->id))}}">{{trans('navmenu.unknown')}}</a></td>
                                    @endif
                                    @if($shop->subscription_type_id == 2)
                                    <td><a href="{{route('pm-purchases.show', encrypt($pmpurchase->id))}}">{{ sprintf('%04d', $pmpurchase->grn_no)}}</a></td>
                                    @endif
                                    <td>{{number_format($pmpurchase->total_amount)}}</td>
                                    <td>{{number_format($pmpurchase->amount_paid)}}</td>
                                    <td>{{number_format($pmpurchase->total_amount-$pmpurchase->amount_paid)}}</td>
                                    <td>{{$pmpurchase->created_at}}</td>
                                    <td>
                                        @if($shop->subscription_type_id == 2)
                                        <a href="{{route('pm-purchases.show', encrypt($pmpurchase->id))}}">
                                            <span class="lni lni-eye"></span>
                                        </a> | @endif 
                                        @if($pmpurchase->amount_paid < $pmpurchase->total_amount)
                                        <a href="#" onclick="showModal('<?php echo $pmpurchase->id; ?>')" data-id="{{$pmpurchase->id}}"><i class="bx bx-money"></i></a> |@endif 
                                        <a href="{{route('pm-purchases.edit', encrypt($pmpurchase->id))}}">
                                            <i class="bx bx-edit" style="color: blue;"></i>
                                        </a> | 
                                        <a href="#" onclick="confirmDelete('<?php echo Crypt::encrypt($pmpurchase->id); ?>')">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel">{{trans('navmenu.add_payment')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="form" method="POST" action="{{route('pm-purchase-payments.store')}}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <input type="hidden" name="pm_purchase_id" id="id_hide">
                           ...
                           @if($settings->allow_multi_currency)
                                ...
                                <div class="col-sm-4" id="rate-mode-col" style="display: none;">
                                ...
                                <div class="col-sm-4" id="locale" style="display: none;">
                                ...
                                <div class="col-sm-4" id="foreign" style="display: none;">
                          ...
                          {{--  @if($shop->subscription_type_id ==3 || $shop->subscription_type_id ==4)
                            <div id="bankdetail" style="display: none;">
                                <div class="form-group col-sm-4" id="deposit_mode" style="display: none;">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input id="name" type="text" name="bank_branch" placeholder="Please enter Bank Branch" class="form-control form-control-sm mb-3">
                                </div>
                                ...
                                <div class="form-group col-sm-6" id="slip" style="display: none;">
                                    <label class="form-label">Credit Card/Bank Slip Number</label>
                                    <input id="name" type="text" name="slip_no" placeholder="Please enter Credit Card/Bank Slip number" class="form-control form-control-sm mb-3">
                                </div>
                            </div>
