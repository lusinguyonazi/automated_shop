                        ...
                        <form class="row g-3"  name="saleform" method="POST" action="{{ route('pos.store') }}" onsubmit="return validateform(this)" ng-if="saletemp">
                            ...
                            <div class="col-sm-12">
                                <table id="discount_field" class="table table-responsive table-striped display nowrap" style="width: 100%; display: block; overflow: scroll; overflow: auto;">
                                    <tr>
                                        <th>#</th>
                                        <!-- <th style="text-align: center;">{{trans('navmenu.product_no')}}</th> -->
                                    ...
                                    <tr ng-repeat="newsaletemp in saletempitems" id="temps">
                                        ...
                                        @if($settings->allow_unit_discount)
                                        <td><input type="number" min="0" step="any" class="form-control form-control-sm mb-1" style="text-align:center; width: 140px;" name="discount" ng-blur="updateSaleTemp(newsaletemp)" ng-model="newsaletemp.discount"></td>
                                        @endif
                                        <td>@{{(newsaletemp.price_per_unit * newsaletemp.quantity_sold) | number:2}}</td>
                                        <td><input type="number" min="0" step="any" class="form-control form-control-sm mb-1" style="text-align:center; width: 140px;" name="total_discount" ng-blur="updateSaleTemp(newsaletemp)" ng-model="newsaletemp.total_discount"></td>
                                        @if($settings->is_vat_registered)
                                ...
                                <table id="discount_value" class="table table-responsive table-striped display nowrap" style="width: 100%; display: none; overflow: scroll; overflow: auto; ">
                                    ...
                                    <tr ng-repeat="newsaletemp in saletempitems" id="temps">
                                        ...
                                        <td>
                                        @if($settings->enable_cpos)
                                            <input type="number" min="0" step="any" style="text-align:center; height: 20px; width: 240px; border: 1px solid #e0e0e0;" name="price_per_unit" ng-blur="updateSaleTemp(newsaletemp)" string-to-number ng-model="newsaletemp.price_per_unit">
                                        @else
                                            @{{newsaletemp.price_per_unit | number:2}}
                                        @endif
                                        </td>
                            ...
                            <div class="row">
                                <div class="col-xl-6">
                                    <div class="row">
                                        <input type="hidden" id="no_items" name="no_items" value="@{{saletempitems.length}}" class="form-control form-control-sm mb-3">
                                ...
                                <div class="col-xl-6">
                                    ...
                                    @if($shop->subscription_type_id == 1)
                                        <tr>
                                            <th>{{trans('navmenu.grand_total')}}</th>
                                        ...
                                        <tr id="total_discount_field" style="display: none;">
                                            ...
                                            <th><input type="number" style="text-align:center;" name="sale_discount" ng-blur="updateSaleTempDiscount(saletempitems)" string-to-number ng-model="total_discount" value="@{{sumDiscount(saletempitems)}}" class="form-control form-control-sm mb-3"></th>
                                        </tr>
                                        <tr id="total_discount_value">
                                            <th>{{trans('navmenu.total_discount')}}</th>
                                            <th style="text-align: right;"><b>@{{sumDiscount(saletempitems) | number:2}}</b></th>
                                        </tr>
                                        <tr>
                                            <th>{{trans('navmenu.total_payable')}}</th>
                                            <th style="text-align: right;"><b>@{{(sum(saletempitems)-sumDiscount(saletempitems)) | number:2}}</b></th>
                                        </tr>
                                        <tr id="paid" style="display: none;">
                                            <th>{{trans('navmenu.paid')}}</th>
                                            <th style="text-align: right;"><b>@{{(sum(saletempitems)-sumDiscount(saletempitems)) | number:2}}</b></th>
                                        </tr>
                                        <tr id="amount_due" style="display: none;">
                                            <th>{{trans('navmenu.amount_due')}}</th>
                                            <th style="text-align: right;">@{{(sum(saletempitems)-sumDiscount(saletempitems))-add_payment | number:2}}</th>
                                        </tr>
                                    @else
                                        <tr>
