            ...
            <div class="card-body">
                <form class="row g-3" method="POST" action="{{route('sales-returns.update', $salereturn->id)}}">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" name="customer" class="form-control form-control-sm mb-3" value="{{$sale->name}}" readonly>
                    </div>
                    <div class="form-group col-md-4">
                        <label class="form-label">Sale Date :</label>
                        <input type="text" name="date" class="form-control form-control-sm mb-3" value="{{date('d, M Y', strtotime($sale->time_created))}}" readonly>
                    </div>
                    <div class="form-group col-md-4" style="text-align: right;">
                        <button class="btn btn-primary">Create</button>
                        <a href="{{url('delete-sale-return/'.encrypt($salereturn->id))}}" class="btn btn-warning">Cancel</a>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <form id="item-form{{$key}}" class="form" method="POST" action="{{route('sale-return-items.update', $item->id)}}">
                                        ...
                                        <td class="qty">
                                        <input type="hidden" name="sale_return_id" value="{{$salereturn->id}}">
                                        {{-- <input type="number" class="form-control form-control-sm mb-1" style="text-align:center; width: 140px;" autocomplete="off" name="quantity" ng-blur="updateSaleTemp(newsaletemp)" string-to-number ng-model="newsaletemp.quantity_sold" min="0" step="any"> --}}
                                        <input id="input_qty{{$key}}"type="number" min="0" oninput="validity.valid||(value='');" name="quantity" value="{{number_format($item->quantity)}}" style="text-align: center;">
                                        <script>
                                            $(function(){
