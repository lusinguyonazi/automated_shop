...
@section('content')
    ...
    <!--end breadcrumb-->
    <div class="row">
        @if(Auth::user()->hasRole('manager')||Auth::user()->can('add-supplier'))
        <div class="col-md-3 mx-auto " >
            <h6 class="mb-0 text-uppercase text-center">{{trans('navmenu.add_new_supplier')}}</h6>
            <hr>
            <div class="card radius-6">
                <div class="card-body">
                    <form class="form row" method="POST" action="{{route('suppliers.store')}}">
                        @csrf
                        <input type="hidden" name="supplier_for" value="Stock">
                        <div class="col-sm-12" >
                            <label for="name" class="form-label">{{trans('navmenu.supplier_name')}}</label>
                            <input type="text" name="name" class="form-control form-control-sm mb-3" required placeholder="{{trans('navmenu.hnt_supplier_name')}}"> 
                        </div>
                        <div class="col-sm-12">
                            <label class="form-label">{{trans('navmenu.contact_number')}}</label>
                            <input id="phone" type="tel" name="contact_no" placeholder="{{trans('navmenu.hnt_contact_number')}}  Eg. 0789XXXXXX" class="form-control form-control-sm mb-3" data-inputmask='"mask": "9999999999"' data-mask>
                        </div> 
                        <div class="col-sm-12">
                            <label class="form-label">{{trans('navmenu.email_address')}}</label>
                            <input type="text" name="email" placeholder="{{trans('navmenu.hnt_supplier_email')}}" class="form-control form-control-sm mb-3">
                        </div>  
                        <div class="col-sm-12">
                            <label class="form-label">{{trans('navmenu.address')}}</label>
                            <input type="text" name="address" placeholder="{{trans('navmenu.address')}}" class="form-control form-control-sm mb-3">
                        </div>
                        <input type="hidden" name="supplier_for" value="Stock">
                    ...
                    <table id="example1" class="table table-responsive mb-0" style="width: 100%;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(Auth::user()->hasRole('manager')||Auth::user()->can('edit-supplier'))
                                    <a href="{{route('suppliers.edit' , encrypt($supplier->id))}}">
                                        <i class="bx bx-edit" style="color: blue;"></i>
                                    </a>@endif
                                    
                                    |
                                    @if(Auth::user()->hasRole('manager')||Auth::user()->can('delete-supplier'))
